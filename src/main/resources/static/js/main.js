function request(method,url,data,successCallBack,errorCallBack,async){
        ...
        $.ajax({
            ...
            data: data,
            method: method
        }).done(successCallBack).fail(errorCallBack || serverError);
}
...
function convert2JsonData(formEl){
	var inputEl =formEl.find('input');
	var data = {};
	inputEl.each(function (i, item) {
...
function redirect(basePath, data){
	document.location.href = basePath + data.nextAction;
}

function addBasePath(endpoint){
